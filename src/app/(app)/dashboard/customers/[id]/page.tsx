...
/**
 * Komponen CustomerDetailPage.
 * Menampilkan detail informasi nasabah, termasuk:
 * - Informasi dasar (usia, pekerjaan, kontak, alamat)
 * - Status deposito dengan dropdown
 * - Skor peluang dan status kontak
 * - Riwayat interaksi dan catatan internal
 *
 * @component
 * @returns {JSX.Element} Halaman detail nasabah.
 */
export default function CustomerDetailPage() {
  const { id } = useParams();
  const router = useRouter();

  // State untuk customer, history, loading, dan catatan baru
  ...
  /**
   * Fetch data customer dari API berdasarkan ID.
   */
  useEffect(() => {
    if (!id) return;
  ...
  /**
   * Mendapatkan kelas warna berdasarkan status kontak.
   */
  ...
  /**
   * Mendapatkan ikon berdasarkan tipe riwayat.
   */
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          >
            ...
            <select
              ...
              onChange={(e) =>
                setCustomer((prev) =>
                  prev ? { ...prev, deposit: e.target.value } : prev
                )
              }
              className={`ml-1 px-1 py-0.5 rounded-lg font-semibold outline-none text-sm ${
          ...
          <div className="lg:col-span-3 flex flex-col gap-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div className="bg-white rounded-xl shadow-md border border-gray-100 p-6 flex flex-col text-center justify-center">
                ...
                <button className="flex items-center justify-center gap-2 bg-[var(--color-accent,#00A884)] hover:bg-[#009970] text-white font-semibold px-4 py-2.5 rounded-lg shadow transition text-sm">
